...
QWidget *multiColumnTreeList()
{
    ...
    auto onUpdate = [tree, spanCheck, selCheck] {
        ...
        QAbstractItemView::SelectionMode selMode = selCheck->isChecked()
...
QWidget *tabsInDirections()
{
    ...
    QTabWidget::TabPosition positions[] = { QTabWidget::North, QTabWidget::South, QTabWidget::West,
                                            QTabWidget::East };
...
QWidget *frames()
{
    ...
    QFrame::Shape shapes[] = { QFrame::NoFrame, QFrame::Box,   QFrame::Panel,      QFrame::WinPanel,
                               QFrame::HLine,   QFrame::VLine, QFrame::StyledPanel };
    QFrame::Shadow shadows[] = { (QFrame::Shadow)QFrame::NoFrame, QFrame::Plain, QFrame::Raised,
                                 QFrame::Sunken };
...
QWidget *tableWithColumns()
{
    ...
    QObject::connect(alwaysScrollHA, &QAction::triggered, w, [w](bool checked) {
        w->setHorizontalScrollBarPolicy(checked ? Qt::ScrollBarAlwaysOn : Qt::ScrollBarAsNeeded);
    });
    ...
    QObject::connect(alwaysScrollVA, &QAction::triggered, w, [w](bool checked) {
        w->setVerticalScrollBarPolicy(checked ? Qt::ScrollBarAlwaysOn : Qt::ScrollBarAsNeeded);
    });
    acts += alwaysScrollHA;
    acts += alwaysScrollVA;
    ...
    QObject::connect(stretchHorizA, &QAction::triggered, w,
                     [w](bool checked) { w->horizontalHeader()->setStretchLastSection(checked); });
    auto stretchVertA = new QAction(w);
    stretchVertA->setText("Stretch Last Vertical");
    stretchVertA->setCheckable(true);
    QObject::connect(stretchVertA, &QAction::triggered, w,
                     [w](bool checked) { w->verticalHeader()->setStretchLastSection(checked); });
    acts += stretchHorizA;
    acts += stretchVertA;
    addSep();
