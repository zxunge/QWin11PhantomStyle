...
using Rgb = QWin11Phantom::Rgb;
using Hsl = QWin11Phantom::Hsl;

Q_NEVER_INLINE static QColor mergedColors(const QColor &colorA, const QColor &colorB,
                                          int factor = 0)
{
...
protected:
    void paintEvent(QPaintEvent *e) override
    {
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            QColor c = QWin11Phantom::lerpQColor(from, to, (double)i / 255.0);
            ...
            double l = QWin11Phantom::lerp(hslf.l, hslt.l, alpha);
...
ColorTest::ColorTest(QWidget *parent) : QWidget(parent)
{
    ...
    connect(slider, &QSlider::valueChanged, bandWidget, [bandWidget, glabel](int val) {
        double v = (double)val / 1000.0;
        v = QWin11Phantom::lerp(0.1, 12.0, v);
        bandWidget->gamma = v;
        bandWidget->update();
